// 📁 app/payphone-success/PayPhoneSuccessClient.tsx
'use client';
...
type PaymentStatus = 'pending' | 'confirming' | 'completed' | 'failed';

export default function PayPhoneSuccessClient() {
    ...
    const loadAssignedNumbers = async (participantIdToUse: string) => {
        try {
            ...
            console.log('📊 Cargando números para participante:', participantIdToUse);
            ...
            if (data.numbers && data.numbers.length > 0) {
                ...
                console.warn('⚠️ No se encontraron números asignados');
            }
        } catch (error) {
    ...
    // Función principal para confirmar la transacción
    const confirmPayPhoneTransaction = async () => {
        if (!payphoneId || !clientTransactionId) {
            console.error('❌ Faltan parámetros de PayPhone');
        ...
        try {
            ...
            console.log('🔄 Confirmando transacción PayPhone:', {
                id: payphoneId,
                clientTransactionId
            });
            ...
            if (data.success && data.status === 'approved') {
                ...
                // Cargar números asignados
                if (data.participantId) {
                    await loadAssignedNumbers(data.participantId.toString());
                } else {
                    console.warn('⚠️ No se recibió participantId');
                }
            } else if (data.status === 'cancelled') {
                console.log('❌ Pago cancelado');
                setPaymentStatus('failed');
            } else {
                console.error('⚠️ Estado desconocido:', data);
                setPaymentStatus('failed');
            }
    ...
    // Effect principal - confirmar al cargar
    useEffect(() => {
        if (payphoneId && clientTransactionId) {
            ...
            <main className="min-h-screen flex flex-col items-center justify-center px-4 bg-gradient-to-br from-indigo-50 to-purple-50">
                <div className="max-w-md w-full bg-white rounded-2xl shadow-2xl p-8">
                    ...
                    <h1 className="text-2xl font-bold text-center text-gray-800 mb-4">
                        {paymentStatus === 'pending' ? 'Validando pago' : 'Confirmando transacción'}
                    </h1>

                    {/* Mensaje */}
                    <div className="bg-indigo-50 border-2 border-indigo-200 rounded-lg p-4 mb-6">
                        <p className="text-center text-gray-700 mb-3">
                            ⏳ Estamos validando tu pago con <strong>PayPhone</strong>
                        </p>
                        <div className="text-sm text-gray-600 space-y-2">
                            <p>• Confirmando la transacción...</p>
                            <p>• Generando tus números de rifa...</p>
                    ...
                    {clientTransactionId && (
                        <div className="bg-gray-50 rounded-lg p-4 space-y-2 text-sm">
                            <div className="flex justify-between">
                                ...
                                <span className="font-semibold text-gray-800">{payphoneId}</span>
                            </div>
                            <div className="flex justify-between">
                                <span className="text-gray-600">Referencia:</span>
                                <span className="font-semibold text-gray-800 text-xs">{clientTransactionId}</span>
                            </div>
                        </div>
                    )}
            ...
            <main className="min-h-screen flex flex-col items-center justify-center px-4 bg-gradient-to-br from-red-50 to-orange-50">
                <div className="max-w-md w-full bg-white rounded-2xl shadow-2xl p-8 text-center">
                    ...
                    <p className="text-gray-600 mb-6">
                        No pudimos confirmar tu pago con PayPhone. Por favor, verifica tu app o intenta nuevamente.
                    </p>
    ...
    // Estado: Pago completado - mostrar números
    return (
        <main className="min-h-screen flex flex-col items-center justify-center px-2 sm:px-4 bg-gradient-to-br from-gray-50 to-gray-100">
            ...
            <div className="mb-6 bg-green-50 border-2 border-green-200 rounded-lg p-4 max-w-md mx-auto">
                ...
                {orderInfo && (
                    <div className="text-xs text-green-700 space-y-1 mt-3">
                        <p><strong>Orden:</strong> {orderInfo.orderNumber}</p>
                        <p><strong>ID Transacción:</strong> {orderInfo.transactionId}</p>
                        <p><strong>Código de autorización:</strong> {orderInfo.authorizationCode}</p>
                    </div>
                )}
            </div>
            ...
            {/* Slider de números asignados (COPIAR EL MISMO CÓDIGO DEL SUCCESS ORIGINAL) */}
