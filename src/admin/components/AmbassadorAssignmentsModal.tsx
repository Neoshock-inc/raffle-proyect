...
export default function AmbassadorAssignmentsModal({ isOpen, onClose, ambassador }: Props) {
    ...
    useEffect(() => {
        if (isOpen && ambassador) {
            setLoading(true)
            numberPoolService.getAssignmentsByAmbassador(ambassador.id)
                .then(setAssignments)
    ...
    return (
        <Modal
            isOpen={isOpen}
            onClose={onClose}
            title={`Asignaciones de ${ambassador.name}`}
            size="lg"
        >
                ...
                <div className="flex gap-4">
                    ...
                    <div className="flex items-center gap-2 bg-indigo-50 dark:bg-indigo-900/30 rounded-lg px-3 py-2">
                        ...
                        <span className="text-sm font-medium text-indigo-700 dark:text-indigo-300">
                            {totalNumbers.toLocaleString()} números asignados
                        </span>
                    </div>
                </div>
                ...
                {loading ? (
                    ...
                    <div className="text-center py-12">
                        ...
                        <p className="text-gray-500 dark:text-gray-400">
                            No tiene asignaciones de números en ninguna rifa
                        </p>
                    </div>
                ) : (
                            ...
                            <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                                {assignments.map(a => (
                                    <tr key={a.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                        ...
                                        <td className="px-4 py-2.5 text-xs text-amber-600 dark:text-amber-400 font-medium">
